    ...
    <div className="h-screen flex relative flex-col text-center md:text-left md:flex-row max-w-7xl px-10 justify-evenly mx-auto items-center">
      ...
      <div className="flex flex-col space-y-4 md:space-y-5 lg:space-y-6 xl:space-y-6 2xl:space-y-10">
        <h4 className="text-xl md:text-2xl lg:text-3xl 2xl:text-4xl font-semibold text-center">
          ...
          <span className="decoration-theme-accent/50 underline">Lets talk.</span>
        </h4>

        <div className="space-y-1 md:space-y-3 lg:space-y-3 xl:space-y-3 2xl:space-y-5">
          <div className="flex items-center space-x-5 justify-center">
            <PhoneIcon className="text-theme-accent h-7 w-7 animate-pulse" />
            <p className="text-lg md:text-2xl lg:text-2xl">{pageInfo.phoneNumber}</p>
          </div>
          <div className="flex items-center space-x-5 justify-center">
            <EnvelopeIcon className="text-theme-accent h-7 w-7 animate-pulse" />
            <p className="text-lg md:text-2xl lg:text-2xl">
              {pageInfo.email}
            </p>
          </div>
          <div className="flex items-center space-x-5 justify-center">
            <MapPinIcon className="text-theme-accent h-7 w-7 animate-pulse" />
            <p className="text-lg md:text-2xl lg:text-2xl">
              {pageInfo.address}
            </p>
          </div>
        ...
        >
          ...
          <textarea
            ...
            className="contactInput"
          />
          <button className="bg-theme-accent py-3 md:py-5 px-10 rounded-lg text-white font-bold text-lg">
            {" "}
            Submit
          </button>
        </form>
